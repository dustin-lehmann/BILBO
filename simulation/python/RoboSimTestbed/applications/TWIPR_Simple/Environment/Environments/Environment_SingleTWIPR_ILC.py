...
from scioi_py_core.objects.twipr.EnvironmentTWIPR_objects import TWIPR_ILC_Agent, standard_reference_trajectory
...
class Environment_SingleTWIPR_ILC(EnvironmentBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.agent1 = TWIPR_ILC_Agent(world=self.world, agent_id=1, r=0.001, s=0.5, reference=standard_reference_trajectory)
...
def main():
    env = Environment_SingleTWIPR_ILC(visualization='babylon', webapp_config={'title': 'Single Agent ILC', 'record': {'file': 'single_agent_ilc.webm', 'time': 92}})
    env.init()
    env.start()
