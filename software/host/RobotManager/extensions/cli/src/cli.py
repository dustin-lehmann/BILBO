        ...
        for name, argument in self.arguments.items():
            if argument.optional or argument.default is not None:
                ...
                if argument.name not in keyword_args.keys():
                    ...
                    return
        try:
            x = self.function(*positional_args, **keyword_args)
            return x
    ...
    def run(self, command_string):
        ...
        while i < len(tokens):
            ...
            if token == '.':
                ...
                if current_set.parent_set is not None:
                    ...
                    logger.error("Already at root, cannot exit further.")
                    return
                i += 1
            else:
                break
        ...
        while i < len(tokens):
            ...
            if token == '.':
                ...
                logger.error(f"Token '{token}' not recognized in set '{current_set.name}'")
                return
...
class CLI:
    ...
    # ==================================================================================================================
    def __init__(self, root_set: CommandSet = None, active_set: CommandSet = None, text_output_function: callable = None):
    ...
    def getCommandSetByPath(self, path: list) -> (None, CommandSet):
        ...
        The token '.' (or a token equal to the current set’s name) is interpreted
        as referring to the current (or root) set.
        """
    ...
    def executeFromConnectorDict(self, command_dict: dict):
        ...
        The method will:
          ...
          4. Fill in any missing optional, default, or flag arguments.
          5. Execute the command’s callback with the provided positional and keyword arguments.
        """
        ...
        # Fill in missing optional/default/flag arguments as in Command.run().
        for arg_name, arg_def in command_obj.arguments.items():
            if (arg_def.optional or arg_def.default is not None) and arg_name not in kw_args:
                kw_args[arg_name] = arg_def.default
            elif arg_def.is_flag and arg_name not in kw_args:
                kw_args[arg_name] = False
            elif not arg_def.optional and arg_def.default is None and not arg_def.is_flag:
                if arg_name not in kw_args:
                    logger.error(f"Required argument '{arg_name}' was not provided for command '{command_name}'.")
                    return

        try:
            # Execute the command's callback with the parsed arguments.
            # ret = command_obj.run(command_path, **kw_args)
            ret = command_obj.function(*pos_args, **kw_args)
            if isinstance(ret, str):
                self.trace(ret)
