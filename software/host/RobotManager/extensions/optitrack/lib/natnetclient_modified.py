...
class NatNetClient:
    ...
    def __createDataSocket(self, port):
        ...
        result.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP,
                          socket.inet_aton(self.multicastIPAddress) + socket.inet_aton(self.localIPAddress))
    ...
    def __unpackRigidBody(self, data):
        ...
        rigid_body_data['position'] = pos
        rigid_body_data['orientation'] = rot
        ...
        if self.natNetStreamVersion[0] >= 2:
            ...
            rigid_body_data['marker_error'] = markerError
        ...
        if (((self.natNetStreamVersion[0] == 2) and (self.natNetStreamVersion[1] >= 6)) or
            ...
            rigid_body_data['tracking_valid'] = trackingValid
    ...
    def __unpackMocapData(self, data):
        ...
        for i in range(0, markerSetCount):
            ...
            marker_set_name = modelName.decode('utf-8')
            mocap_data['marker_sets'][marker_set_name] = {}
            ...
            for j in range(0, markerCount):
                ...
                mocap_data['marker_sets'][marker_set_name][j + 1] = pos  # make index + 1 to have the correct naming
                # scheme
        ...
        mocap_data['rigid_bodies'] = {}

        for i in range(0, rigidBodyCount):
            new_offset, rigid_body = self.__unpackRigidBody(data[offset:])
            offset += new_offset
            mocap_data['rigid_bodies'][rigid_body['id']] = rigid_body
    ...
    @staticmethod
    def __unpackMarkerSetDescription(data):
        offset = 0
        ...
        marker_set['name'] = name.decode('utf-8')

        markerCount = int.from_bytes(data[offset:offset + 4], byteorder='little')

        marker_set['marker_count'] = markerCount
        ...
        for i in range(0, markerCount):
            ...
            marker_set['markers'][i] = name.decode('utf-8')

        return offset, marker_set
    ...
    def __unpackRigidBodyDescription(self, data):
        ...
        if self.natNetStreamVersion[0] >= 2:
            ...
            rigid_body_description['name'] = name.decode('utf-8')

        id = int.from_bytes(data[offset:offset + 4], byteorder='little')
        offset += 4

        rigid_body_description['id'] = id
        ...
        if self.natNetStreamVersion[0] >= 3 or self.natNetStreamVersion[0] == 0:
            ...
            rigid_body_description['marker_count'] = markerCount
            rigid_body_description['markers'] = {}

            markerCountRange = range(0, markerCount)
            for marker in markerCountRange:
                markerOffset = Vector3.unpack(data[offset:offset + 12])
                marker_id = marker + 1  # Start counting at 1
                rigid_body_description['markers'][marker_id] = {}
                rigid_body_description['markers'][marker_id]['id'] = marker_id
                rigid_body_description['markers'][marker_id]['offset'] = markerOffset
                offset += 12
        ...
        return offset, rigid_body_description
    ...
    def __unpackDataDescriptions(self, data):
        ...
        for i in range(0, datasetCount):
            ...
            offset += 4
            if type == 0:
                new_offset, marker_set = self.__unpackMarkerSetDescription(data[offset:])
                offset += new_offset
                marker_sets[marker_set['name']] = marker_set

            elif type == 1:
                new_offset, rigid_body = self.__unpackRigidBodyDescription(data[offset:])
                offset += new_offset
    ...
    def __dataThreadFunction(self, socket):
        while True:
            ...
            try:
                ...
                print("Exception:", e)
                print("Optitrack is not streaming. Enable streaming in Motive. Exit Program")
                os._exit(1)
