...
from utils.network import get_current_user, get_own_hostname, getLocalIP_RPi, has_internet, get_wifi_ssid
...
from robot.utils.display import Display
from robot.utils.display.pages import StatusPage
from utils.singletonlock.singletonlock import terminate, process_running, get_lock_mode
from robot.utils.sound.sound import SoundSystem
...
class Startup:
    ...
    def __init__(self):
        ...
        self.LED_PIN = self.config['pins']['RC_SIDE_BUTTON_LED']
        self.BUTTON_PIN = self.config['pins']['RC_SIDE_BUTTON_PIN']
        ...
        self.sound_system = SoundSystem(volume=0.5)
    ...
    def task(self):
        while not self._exit:
            ...
            if self.long_press_detected:
                self.long_press_detected = False
                if process_running(lock_file='/tmp/twipr.lock'):
    ...
    def update_status_page(self):
        ...
        self.status_page.set_internet_status(has_internet(timeout=1))
