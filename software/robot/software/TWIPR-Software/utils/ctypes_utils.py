...
# def bytes_to_ctype(ctype_type, byte_data):
#     """
#     Convert raw bytes into a ctypes instance.
#
#     Args:
...
#     if len(byte_data) != ctypes.sizeof(ctype_type):
#         raise ValueError(f"byte_data must have exactly {ctypes.sizeof(ctype_type)} bytes.")
#     buffer = ctypes.create_string_buffer(byte_data)
#     return ctypes.cast(ctypes.pointer(buffer), ctypes.POINTER(ctype_type)).contents
